...
ws.run "cmd /c cd /d D:\Frp\frp_0.65.0_windows_amd64\frp_0.65.0_windows_amd64 && frpc.exe -c frpc.toml", vbhide
```
...
5. **测试一下：** 关掉你之前一直开着的那个 FRP 黑框框，然后双击运行这个 `start_frpc.vbs`。你会发现屏幕上没有任何反应（没有黑框弹出），但如果你此时去手机上打开 Moonlight，你会发现居然直接秒连了！这就是后台静默运行的魅力。
...
3. 系统会自动弹出一个文件夹（这叫“启动”文件夹，放在这里面的东西开机都会自动运行）。
    
4. 把刚才做好的那个 **`start_frpc.vbs`** 文件，**剪切并粘贴**到这个“启动”文件夹里。