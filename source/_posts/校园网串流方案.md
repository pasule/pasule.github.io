...
### PC端FRP配置
>在本地下载 Windows 版 FRP(注意版本与服务器端保持一致)

**解压并修改配置文件**
>1. 把下载的 zip 压缩包解压到一个你常用的文件夹（比如 D 盘的某个文件夹）。
>2. 在解压出来的文件夹里，找到 **`frpc.toml`** 这个文件。
>3. 右键 -> 打开方式 -> 记事本。
>4. 把里面的内容替换成下面这段代码

```bash
serverAddr = "你的服务器公网地址"
serverPort = 7000
loginFailExit = false

[[proxies]]
name = "sunshine_tcp_47984"
type = "tcp"
localIP = "127.0.0.1"
localPort = 47984
remotePort = 47984
...
保存，之后我们还要做一些收尾工作。
#### 把 FRP 文件夹加入“白名单”（以 Windows 自带的 Defender 为例）

如果你装了火绒或 360，请在它们的设置里找到“信任区/白名单”，把你frp所在的文件夹加进去。如果是 Windows 自带的安全中心：

1. 点击电脑右下角的盾牌图标，打开 **Windows 安全中心**（或者在开始菜单搜“Windows 安全中心”）。
    
2. 点击 **“病毒和威胁防护”**。
    
3. 在“病毒和威胁防护设置”下，点击 **“管理设置”**。
    
4. 往下滚动到底部，找到“排除项”，点击 **“添加或删除排除项”**。
    
5. 点击 **“添加排除项”** -> 选择 **“文件夹”**，然后选中你的文件夹

#### 设置FRP服务开机自启
>1. 在你电脑随便哪个位置（比如桌面，或者你的 FRP 文件夹里），右键新建一个 **文本文档 (.txt)** 
>2. 打开它，把下面这两行代码**原封不动地复制进去**（这利用了 Windows 自带的 VBS 脚本功能，作用是后台静默执行刚才那段启动代码）：
    
```VBScript
Set ws = CreateObject("Wscript.Shell")
ws.run "cmd /c cd /d "这里输入你自己的frp的文件夹的目录，记得把外面的双引号去掉" && frpc.exe -c frpc.toml", vbhide
```

3. 保存文件，然后关掉记事本。
    
4. 把这个文本文件的名字重命名为 **`start_frpc.vbs`**（注意：后缀名一定要从 `.txt` 改成 `.vbs`。如果你的电脑看不到后缀名，请先在文件夹顶部的“查看”里勾选“文件扩展名”）。
    
5. **测试一下：** 双击运行这个 `start_frpc.vbs`。你会发现屏幕上没有任何反应（没有黑框弹出），但如果你此时去手机上打开 Moonlight，看看能否连接！
    

#### 把脚本加入“开机自启”

现在它能隐身运行了，我们再让它开机自己启动：

1. 按住键盘上的 `Win + R` 键，调出“运行”窗口。
    
2. 输入 **`shell:startup`** 并按回车。
    
3. 把刚才做好的那个 **`start_frpc.vbs`** 文件，**剪切并粘贴**到这个“启动”文件夹里。


### 成果
使用中转服务器，主机连着网线的话，一般延迟是稳定在30ms左右的，当然，这也跟校园网的体质有关系。如果出现*端口未转发*的错误报告的话，检查一下服务器和主机的防火墙设置.Moonlight它一般会设置两个地址，一个局域网一个公网。局域网走不通会自动走公网。

>如果延迟和丢包依然比较高的话，将码率降到10M以下，检查一下上行带宽是否正常。因为校园网有AP隔离和QOS策略

![](assets/校园网串流方案/file-20260406021251202.jpeg)
